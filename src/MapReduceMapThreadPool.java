 ...
 */
public class MapReduceMapThreadPool {
    public static PrintWriter writer;

    static {
        try {
            writer = new PrintWriter("resultsMap.txt", "UTF-8");
    ...
    public MapReduceMapThreadPool() throws FileNotFoundException, UnsupportedEncodingException {
    }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        // Read in arguments
        for (int i = 1; i < args.length; i++) {                    // Loop over each file
            File file = new File(args[i]);
            String name = file.getName();                          // Store name of file as String

            StringBuilder contentsSB = new StringBuilder((int) file.length());
            Scanner scanner = new Scanner(file);
            while (scanner.hasNextLine()) {                        // Loop over each line in file
                contentsSB.append(scanner.nextLine() + "\n");      // Append StringBuilder with new line
            }
            input.put(name, contentsSB.toString());                // Pair and add to HashMap
        }
        ...
        // Calculate and print time taken to map
        final long mapTime = System.currentTimeMillis();
        final long timeTaken = mapTime  - startTime;
        System.out.println("Total Time taken to create map with " + poolSize + " threads: " + timeTaken);
